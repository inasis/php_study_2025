...
class AuthenticationController
{
     ...
     */
    public function login($vars, $requestData): AuthResultDTO
    {
        // DTO 생성
        $authLoginCommand = new AuthLoginCommand(
            $requestData['email'] ?? '', 
            $requestData['password'] ?? ''
        );

        return $this->loginUseCase->login($authLoginCommand);
    }
     ...
     * @return array
     */
    public function renewAccessToken($vars, $requestData): ?array
    {
        if (empty($requestData['refreshToken'])) {
             return null;
        }

        return $this->renewAccessTokenUseCase->renewAccessToken($requestData['refreshToken']);
    }
